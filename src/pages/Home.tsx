...
export function Home() {
  ...
  return (
    <div className="bg-[#1589d6] min-h-screen text-[#4A3B2C] font-sans relative">
      ...
      <header className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className="w-20 h-20 rounded-full bg-[#09b5cc] flex items-center justify-center shadow-md border-4 border-[#bfc216]">
        ...
        <div className="bg-white/90 backdrop-blur-md border border-[#bfc216] rounded-full px-4 py-2 shadow-sm flex items-center gap-3 text-sm">
          <button className="flex items-center gap-2 px-3 py-1.5 hover:bg-[#7dfd27] rounded-full transition">
            <span>📅</span> data, ida e volta
          </button>
          <div className="h-4 w-[1px] bg-gray-300"></div>
          <button className="flex items-center gap-2 px-3 py-1. hover:bg-[#7dfd27] rounded-full transition">
            <span>👤</span> Pessoas
          </button>
          <div className="h-4 w-[1px] bg-gray-300"></div>
          <button className="flex items-center gap-2 px-3 py-1.5 hover:bg-[#7dfd27] rounded-full transition">
          ...
          <button className="bg-[#e4e00d] text-white px-5 py-2 rounded-full font-medium hover:bg-[#2A3615] transition shadow">
            Pesquisar
          </button>
        </div>
      </header>
      ...
      <section className="relative w-full bg-[#1e3a8a] text-white flex flex-col justify-between shadow-md" style={{ backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,0.3), rgba(0,0,0,0.4)), url('https://images.unsplash.com/photo-1571003123894-1f0594d2b5d9?auto=format&fit=crop&w=1920&q=80')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <div className="absolute inset-0 bg-black/20"></div>
        
        <div className="max-w-7xl mx-auto w-full px-6 pt-12 relative z-10">
          <h2 className="text-4xl md:text-5xl font-serif text-blue-900 drop-shadow-md leading-tight">
            Viva momentos <br />inesquecíveis
          </h2>
          <p className="text-xl text-blue-900 font-serif mt-2 drop-shadow">no paraíso.</p>
        </div>

        <div className="relative z-10 w-full overflow-hidden leading-none">
          <svg className="relative block w-full h-12 text-[#154fce]" viewBox="0 0 1200 120" preserveAspectRatio="none">
      ...
      <section className="max-w-5xl mx-auto bg-yellow-100 px-4 py-10 text-center">
        <h3 className="text-3xl font-serif font-bold text-[#36451C] tracking-wide mb-3">Promoções Limitadas</h3>
        <p className="text-sm md:text-base text-[#5c4a38] max-w-2xl mx-auto mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {promos?.map((promo) => (
            <div key={promo.id} className="bg-yellow-100 border border-[#217c05] rounded-4xl p-6 shadow-sm hover:shadow-md transition flex flex-col items-center">
              <span className="text-4xl mb-3">{promo.icon}</span>
              <span className="font-semibold text-lg text-[#36451C]">{promo.title}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 py-12 text-center border-t border-[#E5DFD3]">
        <h3 className="text-2xl font-serif font-bold text-[#36451C] mb-10">Benefícios do Resort</h3>
        
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          <div className="flex flex-col items-center">
            <span className="text-3xl mb-2 text-[#36451C]">👥</span>
            <p className="font-medium text-sm">Crianças até 6 anos grátis.</p>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-3xl mb-2 text-[#36451C]">☕</span>
          ...
          <div className="flex flex-col items-center">
            <span className="text-3xl mb-2 text-[#36451C]">✉️</span>
            <p className="font-medium text-sm">Até 12x sem juros</p>
          </div>
          <div className="flex flex-col items-center">
            <span className="text-3xl mb-2 text-[#36451C]">🌐</span>
            <p className="font-medium text-sm">Lazer completo</p>
          </div>
        </div>

        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 relative items-center">
          <img src="https://images.unsplash.com/photo-1590490360182-c33d57733427?auto=format&fit=crop&w=600&q=80" alt="Quarto" className="rounded-xl shadow-md h-64 object-cover w-full" />
          <img src="https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=600&q=80" alt="Restaurante" className="rounded-xl shadow-md h-64 object-cover w-full" />
      ...
      <section className="bg-[#EFEADB] border-t border-b border-[#E3D9C3] py-10 px-4 relative overflow-hidden">
      ...
      <footer className="bg-[#EFEADB] py-12 px-4 border-t border-[#DED4BD]">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
          ...
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-full bg-[#E5B80B] flex items-center justify-center shadow border-2 border-[#36451C]">
              <span className="text-3xl">☀️</span>
            </div>
            <div>
              <h2 className="text-xl font-black tracking-wider text-[#36451C] uppercase leading-none">SOLARIUM</h2>
              <span className="text-xs font-bold tracking-widest text-[#556B2F] uppercase">RESORT</span>
            </div>
          </div>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button className="w-12 h-12 bg-[#36451C] text-white rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition">
          💬
        </button>
      </div>
